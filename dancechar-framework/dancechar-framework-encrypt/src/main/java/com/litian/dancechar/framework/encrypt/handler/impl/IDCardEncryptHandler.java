...
/**
 * 身份证加密处理（AES加密是例子演示需要，真正的业务换成加密机）
 *
 * @author tojson
 * @date 2022/08/28 11:00
 */
public class IDCardEncryptHandler implements IEncryptHandler {

    @Override
    public EncryptTypeEnum getEncryptType() {
        return EncryptTypeEnum.ID_CARD;
    }
    ...
    public String decrypt(String decryptStr){
        // 真实应用需要换成加密机调用
        return AesUtil.decrypt(decryptStr, KeyIndexEnum.IDCARD);
    }
    ...
    public String encrypt(String encryptStr){
        // 真实应用需要换成加密机调用
        return AesUtil.encrypt(encryptStr, KeyIndexEnum.IDCARD);
    }
}
